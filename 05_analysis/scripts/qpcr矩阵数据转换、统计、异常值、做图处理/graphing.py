...
# ---------- 1. 基础过滤 ----------
valid = (df.groupby(["gene", "batch_id", "group"])
           .size().reset_index(name="n")
           .query("n >= @MIN_SAMPLES_PER_GROUP"))
df = df.merge(valid[["gene", "batch_id", "group"]])

valid_pair = (df.groupby(["gene", "batch_id"])["group"]
                .nunique().reset_index()
                .query("group >= 2"))
df = df.merge(valid_pair[["gene", "batch_id"]])
...
# ---------- 3. 每个基因单独出图 ----------
batches_all = sorted(df["batch_id"].unique())  # 确保所有图 x 轴一致
for gene in df["gene"].unique():
    ...
    for idx, batch in enumerate(batches_all):
        for grp, sign in zip(hue_order, (-1, +1)):      # WT 左，HO 右
            pts = data.query("batch_id == @batch and group == @grp")["log2fc"]
    ...
    for idx, batch in enumerate(batches_all):
        wt = data.query("batch_id == @batch and group == 'WT'")["log2fc"]
        ho = data.query("batch_id == @batch and group == 'HO'")["log2fc"]
